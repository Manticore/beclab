...
def get_kprop_cutoff_trf(state_arr, ksquared_arr, coeff, ksquared_cutoff=None):
    ...
    return Transformation(
        [
            ...
            Parameter('project', Annotation(numpy.int32)),
            Parameter('propagate', Annotation(numpy.int32))],
        """
        ${ksquared.ctype} ksquared = ${ksquared.load_idx}(${', '.join(idxs[2:])});
        ${input.ctype} data;

        %if ksquared_cutoff is not None:
        if (${project} && ksquared > ${dtypes.c_constant(ksquared_cutoff, ksquared.dtype)})
        {
        ...
        if (${propagate})
        {
            data = ${mul}(data, ${coeff}, ksquared * ${dt});
        }
